...
public class CartController extends BaseController {
	...
	public CartController(ProductRepository productService, SaleorderService saleOrderService) {
	...
	public String cartFinish(final ModelMap model, final HttpServletRequest request, final HttpServletResponse response)
		...
		if(cart != null) {
			for (CartItem cartItem : cart.getCartItems()) {
				...
				product.setSoldQuantity(product.getSoldQuantity() + cartItem.getQuanlity());
				productService.save(product);
				saleOrder.addSaleOrderProducts(saleOrderProducts);
			}
		}
		
		saleOrder.setOrderStatus(MyConstants.OrderStatus.PENDING);
	...
	public ResponseEntity<Map<String, Object>> ajax_AddToCart(final Model model, final HttpServletRequest request,
		...
		Product productInDb = productService.findById(cartItem.getProductId()).orElseThrow();
