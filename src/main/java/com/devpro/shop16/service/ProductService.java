...
public class ProductService extends BaseService<Product> {
	...
	public PagerData<Product> search(ProductSearchModel searchModel) {
		...
		String sql = "SELECT * FROM tbl_products p WHERE 1=1 and status = 1";
...
//				sql += " and p.status=1 ";
	...
	public PagerData<Product> findTopSaleProducts() {
		String sql = "select * from tbl_products where id in (select product_id from tbl_saleorder_products group by product_id order by SUM(quaLity) DESC) and status = 1";
